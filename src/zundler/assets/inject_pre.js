...
const myGet = function(arg) {
  ...
  if (window.globalContext?.getParameters && window.location.search === "" && Array.from(this.entries()).length === 0) {
  ...
  const originalResult = originalGet.apply(this, [
    arg
  ]);
...
const observer = new MutationObserver((mutationList)=>{
  ...
  for (const mutation of mutationList){
    if (mutation.type === "childList") {
      ...
      for (const scr of Array.from(target.querySelectorAll("script"))){
