...
const setFavicon = (href: string | undefined): void => {
	if (!href) {
		return;
	}
	const favicon = document.createElement("link");
	favicon.setAttribute("rel", "shortcut icon");
	const href_ = normalizePath(href);
...
const createIframe = (html: string): HTMLIFrameElement => {
...
function deepCopyExcept(obj: any, skipProps: string[]): any {
...
const prepare = (html: string): string => {
	function unicodeToBase64(string: string): string {
...
function main(): void {
	...
	window.addEventListener("popstate", (evnt: PopStateEvent) => {
...
function fromHTML(html: string, trim = true): any {
	const html_ = trim ? html.trim() : html;
...
function setUpPopup(): void {
	...
	popup.querySelectorAll(".content-section").forEach(
		(div: HTMLElement, i: number) => {
			if (i > 0) {
...
async function downloadVirtualFile(path: string): Promise<void> {
	retrieveFile(path, (file) => {
		let data: any;
		if (file.base64encoded) {
